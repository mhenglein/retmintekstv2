...
class TextHighlighter {
    ...
    removeAndStoreFormatting() {
        ...
        do {
            ...
            // Add to array
            if (findIndex > -1)
                this.formatting.push([foundTag, findIndex, endIndex - findIndex + 1]);
            // Update index
    ...
    reconvertTextLight(arrReplacements = this.replacements) {
        arrReplacements.forEach((arr) => {
            ...
            if (regexTest)
                this.text = this.text.replace(replacement, arr[1]); // 1 is the <span>text</span>
        });
        return this;
    }
    findAndReplace() {
